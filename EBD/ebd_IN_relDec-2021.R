...
rawpath <- "EBD/ebd_IN_relDec-2021.txt"
...
rm(.Random.seed)
save.image("EBD/ebd_IN_relDec-2021.RData") 
...
save(data_pmp, file = "EBD/pmp_relDec-2021.RData")

rm(list = setdiff(ls(envir = .GlobalEnv), c("data")), pos = ".GlobalEnv")



### Month of December for monthly challenge
data <- data %>% filter(YEAR == 2021 & MONTH == 12)

rm(.Random.seed)
save.image("EBD/ebd_IN_relDec-2021_DEC.RData")
