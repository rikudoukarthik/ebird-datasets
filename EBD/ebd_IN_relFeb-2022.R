...
rawpath <- "EBD/ebd_IN_relFeb-2022.txt"
...
rm(.Random.seed)
save(data, file = "EBD/ebd_IN_relFeb-2022.RData")
...
save(data_pmp, file = "EBD/pmp_relFeb-2022.RData")

rm(list = setdiff(ls(envir = .GlobalEnv), c("data")), pos = ".GlobalEnv")



### Month of February for monthly challenge
data_mc <- data %>% filter(YEAR == 2022 & MONTH == 2)

rm(.Random.seed)
save(data_mc, file = "EBD/ebd_IN_relFeb-2022_FEB.RData")
